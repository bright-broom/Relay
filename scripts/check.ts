import type {ReportDraft} from '../src/prototype/report';
import type {Snapshot} from '../src/prototype/storage';
import {readFile} from 'node:fs/promises';
import assert from 'node:assert/strict';
import {build,transform} from 'esbuild';
async function load(path:string):Promise<unknown>{const output=await build({entryPoints:[path],bundle:true,write:false,format:'esm',platform:'node'});return import('data:text/javascript;base64,'+Buffer.from(output.outputFiles[0].text).toString('base64'))}
const {tokens,palette,colorRecipes,colorTokens}=await load('src/design/tokens.ts') as typeof import('../src/design/tokens');
const {ja,en,translate}=await load('src/i18n/messages.ts') as typeof import('../src/i18n/messages');
...
for(const key of (Object.keys(ja) as Array<keyof typeof ja>))assert.deepEqual(ja[key].match(/\{\w+\}/g)??[],en[key].match(/\{\w+\}/g)??[],key);
...
const luminance=(hex:string)=>{const v=[1,3,5].map(i=>parseInt(hex.slice(i,i+2),16)/255).map(x=>x<=.04045?x/12.92:((x+.055)/1.055)**2.4);return v[0]*.2126+v[1]*.7152+v[2]*.0722};
const contrast=(a:keyof typeof tokens,b:keyof typeof tokens)=>{const [low,high]=[luminance(tokens[a]),luminance(tokens[b])].sort((x,y)=>x-y);return (high+.05)/(low+.05)};
for(const [a,b] of [['accent','on-accent'],['accent-hover','on-accent'],['accent-pressed','on-accent'],['ink','surface'],['ink','subtle'],['muted','surface'],['muted','subtle'],['muted','input']] as const){
 assert.ok(contrast(a,b)>=4.5,`${a}/${b} text contrast`);
}
for(const [a,b] of [['control','input'],['control','subtle'],['accent','surface'],['accent','subtle']] as const){
...
const {emptyReport,validateReport,saveReport}=await load('src/prototype/report.ts') as typeof import('../src/prototype/report');
const {initialCases}=await load('src/prototype/data.ts') as typeof import('../src/prototype/data');
...
for(const outcome of ['resultAgreed','resultNoAnswer','resultFollowup'] as const){
 const c=fresh();const d:ReportDraft={channel:'channelPhone',outcome,note:'',mode:'record'};
...
const completed=fresh();const completeDraft:ReportDraft={channel:'channelEmail',outcome:'resultCompleted',note:'',mode:'complete'};
...
const other:ReportDraft={channel:'channelOther',outcome:'resultOther',note:' ',mode:'record'};
assert.equal(validateReport(other)!.field,'report');
assert.equal(validateReport({...other,note:'Specific outcome'}),null);
assert.ok(validateReport({...other,channel:'unknown'} as unknown as ReportDraft));
console.log('Zero-typing reports, validation, explicit completion, duplicate completion and unchanged appointments: OK.');

const {restore,createSnapshotWriter,validSnapshot,storageKey}=await load('src/prototype/storage.ts') as typeof import('../src/prototype/storage');
const snapshot:Snapshot={locale:'ja',cases:structuredClone(initialCases),drafts:{4:{channel:'channelPhone',outcome:'resultAgreed',note:'draft',mode:'record'}},review:'pending',reviewOwner:'Owner',reviewDue:'dueNow',imported:false};
...
const manifest=JSON.parse(await readFile('prototype/manifest.webmanifest','utf8')) as {display:string;start_url:string;scope:string;theme_color:string;icons:{purpose:string}[]};
...
const config=JSON.parse(await readFile('vercel.json','utf8')) as {outputDirectory:string;routes:{src:string;dest?:string;headers?:Record<string,string>}[]};assert.equal(config.outputDirectory,'public');
...
const events:Record<string,(event:{waitUntil:(job:Promise<unknown>)=>void})=>void>={};const keys=new Set(['unrelated-cache','relay-shell-/-old']);let claimed=false,skipped=false,navigated=false;let job:Promise<unknown>|undefined;
runInNewContext(await readFile('prototype/sw.js','utf8'),{self:{registration:{scope:'https://relay.test/'},addEventListener:(name:string,fn:typeof events[string])=>events[name]=fn,skipWaiting:async()=>{skipped=true},clients:{claim:async()=>{claimed=true},matchAll:async()=>[{url:'https://relay.test/',navigate:async()=>{navigated=true}}]}},caches:{keys:async()=>[...keys],delete:async (key:string)=>keys.delete(key)},URL});
...
const {navigationItems,visibleNavigation,activeNavigation,resolveRoute}=await load('src/prototype/navigation.ts') as typeof import('../src/prototype/navigation');
