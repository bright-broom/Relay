...
import {fixtureFetch} from './support.ts';
...
window.scrollTo=()=>{};
window.matchMedia = () => ({
  matches: false,media:"",onchange:null,addListener(){},removeListener(){},dispatchEvent(){return true;},
  addEventListener() {},
  removeEventListener() {},
});
...
const api = await import(
  pathToFileURL(process.cwd() + "/.vercel/check-react/index.mjs").href
) as (typeof import('../src/app/feature-boundary') & typeof import('../src/app/app') & typeof import('../src/app/root') & typeof import('../src/app/admin') & typeof import('../src/app/store') & typeof import('../src/app/scheduling') & typeof import('../src/app/pricing-results') & typeof import('../src/pricing/comparison') & typeof import('../src/app/my-page') & typeof import('../src/i18n/context') & typeof import('../src/components/ui/tooltip'));
...
await user.selectOptions(
  screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t("recordMethod")),
  "channelPhone",
);
await user.selectOptions(
  screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t("recordOutcome")),
  "resultAgreed",
);
await user.click(
  screen.getByRole<HTMLButtonElement>("button", { name: t("saveRecord") }),
);
...
const pricingTrigger = screen.getByRole<HTMLButtonElement>("button", {
  name: t("pricing"),
});
...
const field = (label:MessageKey) => within(dialog).getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t(label));
const input = (label:MessageKey, value:string) =>
  fireEvent.change(field(label), { target: { value } });
await within(dialog).findByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t("pricingCurrent"));
assert.equal(field("pricingCurrent").value, "");
assert.equal(
  within(dialog).getByRole<HTMLButtonElement>("button", { name: t("pricingPresent") }).disabled,
  true,
);
...
assert.equal(within(dialog).queryByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t("pricingPayment")), null);
await user.selectOptions(
  term,
  within(term).getByRole("option", {
    name: t("pricingMonths", { count: 60 }),
...
input("pricingCurrent", "20000");
await user.click(field("pricingConfirm"));
await user.click(within(dialog).getByRole<HTMLButtonElement>('button',{name:t('pricingAssumptions')}));
document.querySelector<HTMLElement>('.dialog-body')!.scrollTop = 600;
await user.click(
  within(dialog).getByRole<HTMLButtonElement>("button", { name: t("pricingPresent") }),
);
assert.ok(document.querySelector<HTMLElement>(".app.pricing-presenting")!);
assert.equal(document.activeElement!.id,'price-monthly-title','Presenting starts at the monthly comparison');
assert.equal(document.querySelector<HTMLElement>('.dialog-body')!.scrollTop,0);
assert.equal(within(dialog).getByRole<HTMLButtonElement>('button',{name:t('pricingAssumptions')}).getAttribute('aria-expanded'),'false','Presentation initially hides detailed formulas');
assert.ok(within(dialog).getByRole('heading',{name:t('pricingConditionsHeading')}));
await user.click(
  within(dialog).getByRole<HTMLButtonElement>("button", { name: t("pricingEdit") }),
);
...
await user.keyboard("{Escape}");
await user.click(
  screen.getByRole<HTMLButtonElement>("button", { name: t("language") }),
);
await user.selectOptions(
  screen.getByRole<HTMLSelectElement>("combobox", { name: t("language") }),
  "ar",
);
await user.click(
...
await user.click(screen.getByRole<HTMLButtonElement>("button", { name: t("scheduleFind") }));
await screen.findByRole("button", { name: t("scheduleBook") });
await user.selectOptions(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t("scheduleDuration")), "30");
assert.equal(
...
assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t("scheduleDuration")).value, "30");
assert.equal(
  screen.getByRole<HTMLButtonElement>("button", { name: t("scheduleFind") }).disabled,
  false,
);
assert.equal(
...
await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('myNotifications')}));
await user.selectOptions(
  screen.getByRole<HTMLSelectElement>("combobox", { name: t("lineDestination") }),
  "group",
);
await user.click(screen.getByRole<HTMLButtonElement>("button", { name: t("lineConnect") }));
await screen.findByText("fixture-code");
assert.equal(
  screen.getByRole<HTMLSelectElement>("combobox", { name: t("lineDestination") }).value,
  "group",
);
...
await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('myNotifications')}));
assert.equal(screen.getByRole<HTMLSelectElement>('combobox',{name:t('lineDestination')}).value,'group');
...
assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('signOut')}).disabled,false,'Notification failures leave sign-out usable');
failLine = false;
cleanup();
...
adminStatus=403;
await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('refreshConnections')}));
...
adminStatus=401;
await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('refreshConnections')}));
await screen.findByRole<HTMLAnchorElement>('link',{name:t('googleSignIn')});
...
globalThis.fetch=fixtureFetch(async()=>{localAdminRequests++;throw new Error('Local preview must not authenticate');});
render(createElement(api.TooltipProvider,null,createElement(api.Admin,{ui})));
await screen.findByRole('heading',{name:t('adminGateTitle')});
...
for(const locale of ['ja','ar-EG']) {
 ...
 const link=await screen.findByRole<HTMLAnchorElement>('link',{name:localUi.t('googleSignIn')});
...
let pendingResolve!: (response:Pick<Response,'ok'|'status'|'json'>)=>void, dispose!: ()=>void;
const pending:{signal?:AbortSignal|null}={};
globalThis.fetch=fixtureFetch(async()=>({ok:true,status:200,json:async()=>({viewer:'admin@example.test',accounts:[{email:'private@example.test',role:'member',sessions:1}],configuration:{google:true,database:true,line:false,calendar:false}})}));
...
await act(async()=>{dispose=api.mountWorkspace(host,adminWorkspace);});
await screen.findByText('private@example.test');
globalThis.fetch=fixtureFetch(async(_url,options)=>{pending.signal=options.signal;return new Promise(resolve=>{pendingResolve=resolve;});});
await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('refreshConnections')}));
assert.ok(pending.signal);
await act(async()=>{host.dataset.sessionBlocked='true';window.dispatchEvent(new window.Event('relay-session-invalidated'));});
...
for (const locale of ['ja','en','ar-EG']) {
  ...
  for (const scenario of [
    ...
    assert.equal(container.querySelector<HTMLElement>('.price-option[data-phase=before] .price-amount')!.textContent,display.money(input.currentMonthly));
    assert.equal(container.querySelector<HTMLElement>('.price-option[data-phase=after] .price-amount')!.textContent,display.money(result.monthlyDuring));
    const later=container.querySelector<HTMLElement>('.price-later .price-secondary')!;
    ...
    assert.equal(container.querySelector<HTMLElement>('.price-change')!.dataset.direction,monthly);
    assert.equal(container.querySelector<HTMLElement>('.price-total-difference')!.dataset.direction,total);
    const rows=container.querySelectorAll('.price-bar-row');
    assert.equal(rows[0].querySelector<HTMLElement>('dd')!.textContent,display.money(result.currentTotal));
    assert.equal(rows[1].querySelector<HTMLElement>('dd')!.textContent,display.money(result.proposedTotal));
    const widths=[...container.querySelectorAll<HTMLElement>('.price-bar-fill')].map(el=>parseFloat(el.style.getPropertyValue('--comparison-share')));
    ...
    if(remaining) assert.equal(remaining.querySelector<HTMLElement>('bdi')!.textContent,display.money(result.remainingInstallments));
    assert.equal(container.querySelector<HTMLElement>('.price-source')!.hidden,false);
    assert.ok(screen.getByText(display.t('pricingEstimate')));
    assert.ok(screen.getByText(display.t('pricingScope')));
    const disclosure=screen.getByRole<HTMLButtonElement>('button',{name:display.t('pricingAssumptions')});
...
dom.reconfigure({url:'https://relay.test/'});
let guestRequests=0;
globalThis.fetch=fixtureFetch(async()=>{guestRequests++;throw Error('Guest view must not request private APIs');});
for(const page of ['today','cases','reviews','imports','mypage','admin']) {
 ...
 if(page==='today') {
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('pricing')}));
...
for(const label of ['pricing','scheduling','language','previewInfo','handoff'] as const) {
 const trigger=screen.getByRole<HTMLButtonElement>('button',{name:t(label)});
...
const infoTrigger=screen.getByRole<HTMLButtonElement>('button',{name:t('previewInfo')});
await user.click(infoTrigger);
const resetTrigger=screen.getByRole<HTMLButtonElement>('button',{name:t('resetDemo')});
...
await user.click(document.querySelector<HTMLElement>('[data-slot="alert-dialog-overlay"]')!);
...
let mounts=0;
const Slow=lazy(()=>new Promise<{default:()=>ReactNode}>(resolve=>{finish=resolve;}));
const boundary=(child:ReactNode,key='feature')=>createElement('div',null,
 createElement('button',null,'Synthetic navigation'),
 createElement(api.FeatureBoundary,{ui,key,children:child}));
const slowView=render(boundary(createElement(Slow)));
assert.ok(screen.getByText(t('loading')));
assert.ok(screen.getByRole<HTMLButtonElement>('button',{name:'Synthetic navigation'}));
...
const Failed=lazy(()=>new Promise<{default:()=>ReactNode}>((_,reject)=>{fail=reject;}));
const failedView=render(boundary(createElement(Failed)));
assert.ok(screen.getByText(t('loading')));
const captured:unknown[][]=[];const previousError=console.error;
console.error=(...args)=>captured.push(args);
try {
 await act(async()=>{fail(Error('Synthetic chunk download failure'));});
 assert.ok(screen.getByText(t('featureLoadFailed')));
 assert.ok(screen.getByRole<HTMLButtonElement>('button',{name:t('reloadPage')}));
 assert.ok(screen.getByRole<HTMLButtonElement>('button',{name:'Synthetic navigation'}));
