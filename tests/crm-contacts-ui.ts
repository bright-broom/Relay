...
export async function verifyContactsUi({mount,screen,user,waitFor,cleanup,act,t,workspace,customer}: CrmUiContext) {
  let contacts:Array<{id:string;displayName:string;email:string|null;phone:string|null;version:string}>=[],posts:Array<Record<string,unknown> & {contact:Record<string,unknown>}>=[],mode='lost',listFailure=true,viewer=false,archived=false,paginate=false;
  const saved=new Map();
  ...
  const add=async(name:string)=>{
    await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmAddContact')}));
    await user.type(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')),name);
  };
  await open();
  ...
  await user.type(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactEmail')),'synthetic@example.invalid');
  await user.type(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPhone')),'+81 3 1234 5678');
  assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('crmEdit')}).disabled,true);
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmSearchName')).disabled,true);
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmWorkspace')).disabled,true);
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContact')}));
  await screen.findByText(t('crmFailure'));
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).value,'Synthetic private contact');
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactEmail')).disabled,true);
  ...
  await user.selectOptions(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPriority')),'primary');
  mode='primary';
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContact')}));
  await screen.findByText(t('crmPrimaryConflict'));
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).value,'Retained primary draft');
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPriority')).disabled,false);
  const rejectedKey=posts.at(-1)!.key;
  await user.selectOptions(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPriority')),'standard');
  mode='saved';
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContact')}));
  ...
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmMoreContacts')}));
  await screen.findByRole('heading',{name:'Retained primary draft'});
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).value,'Concurrent archive draft');
  ...
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).value,'Concurrent archive draft');
  assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContact')}).disabled,true);
