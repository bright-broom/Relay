import {fixtureFetch, type FixtureFetch} from './support.ts';
import type {CrmUiContext} from './support.ts';
import assert from 'node:assert/strict';

export async function verifyContactEditUi({mount,screen,user,waitFor,cleanup,act,t,workspace,customer}: CrmUiContext) {
  ...
  let archived=false,viewer=false,denied=false,lose=true,failList=false,failLatest=false,patches:Array<Record<string,unknown> & {contact:Record<string,unknown>}>=[];
  const saved=new Map();
  const fixture:FixtureFetch=async(path,options)=>{
  ...
  const open=async()=>{
    globalThis.fetch=fixtureFetch(fixture);mount();
    await user.click(await screen.findByRole('button',{name:t('crmDetails')}));
    await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmLoadContacts')}));
    await screen.findByRole('heading',{name:contact.displayName});
  };
  const edit=async()=>{
    await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmEditContact')}));
    await screen.findByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName'));
  };
  await open();await edit();
  assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('crmEdit')}).disabled,true);
  assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('crmAddContact')}).disabled,true);
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmWorkspace')).disabled,true);
  assert.equal(screen.queryByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPriority')),null);
  await user.clear(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')));
  await user.type(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')),'Edited private contact');
  await user.clear(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactEmail')));
  await user.clear(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPhone')));
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContactChanges')}));
  await screen.findByText(t('crmFailure'));
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).value,'Edited private contact');
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).disabled,true);
  assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('crmCancelEdit')}).disabled,true);
  failList=true;
  ...
  await edit();
  await user.clear(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')));
  await user.type(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')),'Keep my conflicting draft');
  ...
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).value,'Keep my conflicting draft');
  assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContactChanges')}).disabled,true);
  failLatest=false;
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmLoadLatest')}));
  await screen.findByText('Other editor latest');
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactEmail')).value,'');
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmUseLatest')}));
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')).value,'Other editor latest');
  assert.equal(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactEmail')).value,'newer@example.invalid');
  assert.ok(screen.getByRole('heading',{name:'Other editor latest'}),'review refreshes the displayed list record');
  await user.clear(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPhone')));
  await user.type(screen.getByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactPhone')),'555 0111');
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContactChanges')}));
  ...
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmUseLatest')}));
  assert.equal(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContactChanges')}).disabled,true);
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmCancelEdit')}));
  cleanup();archived=false;
  await open();await edit();denied=true;
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContactChanges')}));
  await screen.findByText(t('crmReadOnly'));
  assert.equal(screen.queryByLabelText<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>(t('crmContactName')),null);
  ...
  globalThis.fetch=fixtureFetch(async()=>({ok:true,status:200,json:()=>new Promise(resolve=>{finish=resolve;})}));
  await user.click(screen.getByRole<HTMLButtonElement>('button',{name:t('crmSaveContactChanges')}));
