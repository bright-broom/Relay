...
const api=await import(pathToFileURL(process.cwd()+'/.vercel/check-inputs/index.mjs').href) as (typeof import('../src/ui/controls') & typeof import('../src/ui/input-values') & typeof import('../src/ui/language') & typeof import('../src/i18n/context') & typeof import('../src/pricing/form') & typeof import('../src/pricing/comparison'));
...
for(const kind of ['quote','invoice','priceList'] as const){
...
for(const locale of ['ja','en-US','ar-EG']){
 ...
 assert.equal(new JSDOM(markup).window.document.querySelector<HTMLSelectElement>('#language-tag')!.value,ui.locale);
