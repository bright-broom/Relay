...
import {verifyContacts} from './crm-contacts.ts';
import {verifyContactEdits} from './crm-contact-edit.ts';
...
const api = await import(pathToFileURL(process.cwd()+'/.vercel/check-crm/server.mjs').href) as (typeof import('../src/server/crm') & typeof import('../src/server/crm-contacts') & typeof import('../src/server/auth') & {handle: typeof import('../src/server/handler')['handle']});
const pg = new PGlite();
const wrap = pgliteDatabase;
const ownerDb = wrap(pg);
const db: Database = {query:()=>{throw Error('CRM must use a transaction');}, transaction:work=>pg.transaction(async tx=>{
  await tx.exec('SET LOCAL ROLE relay_crm_runtime');
  return work(wrap(tx));
})};
const id = (n: number) => `00000000-0000-4000-8000-${String(n).padStart(12,'0')}`;
const identity = (n: number) => ({subject:`synthetic-${n}`,email:`synthetic${n}@gmail.com`});
const reject = (work:()=>Promise<unknown>,status:number,code?:string) => assert.rejects(work,e=>errorFields(e).status===status && (!code || errorFields(e).code===code));
const sqlReject = (work:()=>Promise<unknown>) => assert.rejects(work,e=>['42501','55000'].includes(errorFields(e).code ?? ''));
try {
  ...
  for (const n of [1,2,3,4]) {
    await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.principals(id,issuer,subject,display_name,verified_email) VALUES($1,'https://accounts.google.com',$2,'Synthetic person',$3)`,[id(n),identity(n).subject,identity(n).email]);
  }
  for (const n of [11,12]) await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.workspaces(id,name) VALUES($1,$2)`,[id(n),`Synthetic workspace ${n}`]);
  for (const [workspace,person,role] of ([[11,1,'editor'],[11,2,'viewer'],[12,3,'admin']] as const)) await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.memberships(workspace_id,principal_id,role,status) VALUES($1,$2,$3,'active')`,[id(workspace),id(person),role]);

  // Preserve a real pre-upgrade row while adding lifecycle and contacts tables.
  const legacyId = randomUUID();
  await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.customers(id,workspace_id,kind,display_name,name_search,status) VALUES($1,$2,'individual','Legacy synthetic','legacy synthetic','prospect')`,[legacyId,id(12)]);
  ...
  await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.request_dedup(workspace_id,actor_id,operation,key,payload_hash,response_status,result_id,expires_at)
  ...
  await pg.query<Record<string, unknown>>('DELETE FROM relay_crm.request_dedup WHERE key=$1',[legacyInput.key]);
  await pg.query<Record<string, unknown>>('DELETE FROM relay_crm.customers WHERE id=$1',[legacyId]);
  ...
  const count = (table: string) => pg.query<{n:number}>(`SELECT count(*)::int AS n FROM relay_crm.${table}`).then(result=>result.rows[0].n);
  assert.equal(await count('customers'),1);
  assert.equal(await count('request_dedup'),1);
  assert.equal((await pg.query<{n:number}>(`SELECT count(*)::int AS n FROM relay_crm.audit_events WHERE action='create'`)).rows[0].n,1);
  assert.ok(!(await pg.query<Record<string, unknown>>('SELECT changes FROM relay_crm.audit_events')).rows.some(r=>JSON.stringify(r).includes('Synthetic Customer')));
  ...
  const failing: Database = {query:db.query,transaction:work=>db.transaction(tx=>work({...tx,query:<T extends Row>(text: string,values?: (string|number|boolean|null)[])=>{
  ...
  assert.equal(await count('request_dedup'),1);
  const failedDedup: Database = {query:db.query,transaction:work=>db.transaction(tx=>work({...tx,query:<T extends Row>(text: string,values?: (string|number|boolean|null)[])=>{
  ...
  const events = (await pg.query<Record<string, unknown>>("SELECT action,changes FROM relay_crm.audit_events WHERE action IN ('edit','archive','restore') ORDER BY created_at")).rows;
  ...
  for (const broken of [failing,failedDedup]) {
    ...
    assert.equal((await pg.query<Record<string, unknown>>('SELECT version::text FROM relay_crm.customers WHERE id=$1',[customerId])).rows[0].version,'4');
    assert.equal(await count('audit_events'),changesBefore); assert.equal(await count('request_dedup'),keysBefore);
  }
  await pg.query<Record<string, unknown>>("UPDATE relay_crm.memberships SET role='viewer' WHERE principal_id=$1",[id(1)]);
  await reject(()=>api.changeCustomer(identity(1),customerId,edit,db),403,'crmReadOnly');
  await pg.query<Record<string, unknown>>("UPDATE relay_crm.memberships SET role='editor' WHERE principal_id=$1",[id(1)]);

  // Revocation applies to reads AND retries, even when the original save succeeded.
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.memberships SET status='suspended' WHERE principal_id=$1`,[id(1)]);
  ...
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.memberships SET status='active' WHERE principal_id=$1`,[id(1)]);
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.principals SET disabled_at=now() WHERE id=$1`,[id(1)]);
  await reject(()=>api.getCustomer(identity(1),id(11),created.customer.id,db),404);
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.principals SET disabled_at=NULL WHERE id=$1`,[id(1)]);
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.workspaces SET archived_at=now() WHERE id=$1`,[id(11)]);
  await reject(()=>api.getCustomer(identity(1),id(11),created.customer.id,db),404);
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.workspaces SET archived_at=NULL WHERE id=$1`,[id(11)]);
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.request_dedup SET created_at=now()-interval '8 days',expires_at=now()-interval '1 day'`);
  await reject(()=>api.createCustomer(identity(1),input,db),409,'crmRetryConflict');
  ...
  const scoped = <T>(person:number,workspace:number,work:(db:Database)=>Promise<T>) => db.transaction(async tx=>{
  ...
  for (let n=0;n<52;n++) await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.customers(id,workspace_id,kind,display_name,name_search,status,updated_at)
  ...
  await pg.query<Record<string, unknown>>(`INSERT INTO relay_private.sessions(token_hash,subject,email,expires_at) VALUES($1,$2,$3,now()+interval '1 hour')`,[api.hash(token),identity(1).subject,identity(1).email]);
  const request = (route:string,method='GET',body?:unknown,extra:Record<string,string>={})=>new Request('https://relay.test/api/relay?route='+route,{method,headers:{cookie:api.cookie(api.sessionCookie,token,100),origin:'https://relay.test',...extra},...(body!==undefined?{body:JSON.stringify(body)}:{})});
  const call = (req:Request)=>api.handle(req,ownerDb,undefined,db);
  ...
  await pg.query<Record<string, unknown>>('UPDATE relay_crm.customers SET version=9007199254740993 WHERE id=$1',[customerId]);
  ...
  const add = (name:string,person=1,workspace=11)=>api.createCustomer(identity(person),{workspaceId:id(workspace),key:randomUUID(),customer:{displayName:name,kind:'organization'}},db);
  ...
  const search = (query:unknown,extra:Record<string,unknown>={})=>api.searchCustomers(identity(1),{workspaceId:id(11),query,...extra},db);
  ...
  await pg.query<Record<string, unknown>>("UPDATE relay_crm.memberships SET status='suspended' WHERE principal_id=$1",[id(2)]);
  await reject(()=>api.searchCustomers(identity(2),{workspaceId:id(11),query:'unique'},db),404);
  await pg.query<Record<string, unknown>>("UPDATE relay_crm.memberships SET status='active' WHERE principal_id=$1",[id(2)]);

  // All matching records span pages; filters cannot silently reuse a different cursor.
  for(let n=0;n<51;n++) await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.customers(id,workspace_id,kind,display_name,name_search,status,updated_at)
  ...
  const listAudits=(await pg.query<{changes:{searched?:boolean}}>("SELECT changes FROM relay_crm.audit_events WHERE action='list'")).rows;
