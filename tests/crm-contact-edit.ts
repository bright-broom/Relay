...
export async function verifyContactEdits({api,pg,db,id,identity,reject,scoped,sqlReject,call,request,failing,failedDedup}: CrmContext) {
  ...
  const edit=(body:unknown=input,person=1,target=db,contactId=original.id)=>api.editContact(identity(person),contactId,body,target);
  ...
  await assert.rejects(()=>edit(),e=>errorFields(e).code==='42501','missing migration must not permit an edit');
  ...
  const counts=async()=>(await pg.query<Record<string, unknown>>(`SELECT (SELECT count(*) FROM relay_crm.audit_events)::int AS audit,
  ...
  for (const broken of [failing,failedDedup]) {
    ...
    const row=(await pg.query<Record<string, unknown>>('SELECT display_name,email,phone,version::text FROM relay_crm.contacts WHERE id=$1',[original.id])).rows[0];
  ...
  await pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.customer_contacts(workspace_id,customer_id,contact_id,relationship,is_primary) VALUES($1,$2,$3,'contact',false)`,[id(11),second.id,original.id]);
  ...
  for(const table of ['memberships','principals','workspaces']) {
    ...
    await pg.query<Record<string, unknown>>(sql,[table==='memberships'?'suspended':'2026-09-17T00:00:00Z',key]);
    await reject(()=>edit(),404);await reject(()=>read(),404);
    await pg.query<Record<string, unknown>>(sql,[table==='memberships'?'active':null,key]);
  }
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.request_dedup SET created_at=now()-interval '9 days',expires_at=now()-interval '1 day' WHERE operation='contact.edit' AND key=$1`,[input.key]);
  await reject(()=>edit(),409,'crmRetryConflict');
  await pg.query<Record<string, unknown>>('UPDATE relay_crm.contacts SET version=9007199254740993 WHERE id=$1',[original.id]);
  ...
  const audits=(await pg.query<Record<string, unknown>>("SELECT changes FROM relay_crm.audit_events WHERE entity_type='contact' AND action='edit' AND entity_id=$1 ORDER BY created_at",[original.id])).rows;
  ...
  await pg.query<Record<string, unknown>>('UPDATE relay_crm.contacts SET archived_at=now() WHERE id=$1',[original.id]);
