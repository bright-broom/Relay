...
import type {CrmContext} from './support.ts';
...
export async function verifyContacts({api,pg,db,id,identity,reject,scoped,sqlReject,call,request,failing,failedDedup}: CrmContext) {
  ...
  const create = (body:unknown=input,person=1,target=db)=>api.createContact(identity(person),body,target);
  const list = (cursor:string|null=null,person=1,workspace=11,customer=parent.id,target=db)=>api.listContacts(identity(person),id(workspace),customer,cursor,target);
  const counts = async()=> (await pg.query<Record<string, unknown>>(`SELECT (SELECT count(*) FROM relay_crm.contacts)::int AS contacts,
  ...
  const stored = (await pg.query<Record<string, unknown>>('SELECT email_search,phone_search,version::text FROM relay_crm.contacts WHERE id=$1',[first.contact.id])).rows[0];
  ...
  for (const broken of [failing,failedDedup,{query:db.query,transaction:work=>db.transaction(tx=>work({...tx,query:<T extends Row>(text: string,values?: (string|number|boolean|null)[])=>{
  ...
  await assert.rejects(()=>scoped(1,11,tx=>tx.query(`INSERT INTO relay_crm.customer_contacts(workspace_id,customer_id,contact_id,relationship) VALUES($1,$2,$3,'contact')`,[id(11),otherParent.id,first.contact.id])),e=>['42501','23503'].includes(errorFields(e).code ?? ''));
  ...
  await pg.query<Record<string, unknown>>(`UPDATE relay_crm.request_dedup SET created_at=now()-interval '9 days',expires_at=now()-interval '1 day' WHERE operation='contact.create' AND key=$1`,[input.key]);
  ...
  const audits = (await pg.query<{changes:Record<string,unknown>}>("SELECT changes FROM relay_crm.audit_events WHERE entity_type='contact'")).rows;
