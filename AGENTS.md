...
- 一般画面・料金シミュレーターは架空データの公開デモとして提供する。管理情報・外部連携APIは認証必須のまま維持する。公開デモと認証済みの下書き保存を分離し、アプリ本体・APIをオフラインキャッシュしない。公開静的ファイルは `public/` へ明示的に選別する。サーバーの正本は `src/server/`、Vercel の入口は `api/relay.ts`。`prototype/`、`public/`、`dist/server/` はビルド時に生成し、Git 管理しない。ホワイトリストと秘密情報はサーバー環境変数のみ。LINE通知先は本人の連携コード・署名付きWebhook・アプリでの確定を経由し、クライアントから送信先IDや任意本文を受け取らない。
