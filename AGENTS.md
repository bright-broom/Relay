...
- 認証導入後はアプリ本体・APIをオフラインキャッシュしない。公開静的ファイルは `public/` へ明示的に選別する。サーバーの正本は `src/server/`、`api/relay.mjs` は生成物。ホワイトリストと秘密情報はサーバー環境変数のみ。LINE通知先は本人の連携コード・署名付きWebhook・アプリでの確定を経由し、クライアントから送信先IDや任意本文を受け取らない。
