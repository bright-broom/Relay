...
- アプリ・API・計算・テスト・ビルド・運用スクリプトは TypeScript を必須とし、strict な型検査を使う。手書きの `.js` / `.mjs` / `.cjs` は追加しない。
...
- 一般画面・料金シミュレーターは架空データの公開デモとして提供する。管理情報・外部連携APIは認証必須のまま維持する。公開デモと認証済みの下書き保存を分離し、アプリ本体・APIをオフラインキャッシュしない。公開静的ファイルは `public/` へ明示的に選別する。サーバーの正本は `src/server/`、Vercel の入口は `api/relay.ts`。`prototype/`、`public/`、`.vercel/server/` はビルド時に生成し、Git 管理しない。ホワイトリストと秘密情報はサーバー環境変数のみ。LINE通知先は本人の連携コード・署名付きWebhook・アプリでの確定を経由し、クライアントから送信先IDや任意本文を受け取らない。
...
- `prototype/index.html` はデザイン検討用。描画は `src/app/`、共通部品は `src/components/ui/` と `src/ui/`、起動・サンプルデータ・保存互換は `src/prototype/`、中央トークンは `src/design/tokens.ts`、UI文言は `src/i18n/locales/`、共通の翻訳・書式は `src/i18n/context.ts`。生成物を直接編集せず `npm run check` で検証・生成し、`npm run check:reproducible` で再生成の一致を確認する。描画はReactへ移行済み。[共通部品](docs/COMPONENTS.md)を読み、HTML文字列描画や独自の入力・ダイアログを再導入しない。
