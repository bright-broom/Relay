...
案件画面は架空データのプロトタイプです。Googleホワイトリスト認証とLINE個人・グループ通知の接続基盤を実装しました。一般画面と料金シミュレーターはログインなしの公開デモとして利用できます。管理情報・LINE通知・カレンダー連携の利用にはOAuth・PostgreSQL・Messaging APIの接続設定が必要です。案件のサーバー同期・AI接続・自動期限通知は未実装です。
...
架空データによる案件一覧・更新案の確認・対応記録・タスク完了・引き継ぎ文作成を試せます。未ログイン時の記録と下書きはこのタブのsessionStorageへ保存し、認証済み利用者の下書きとは分離します。情報メニューからサンプルを初期化できます。

iOS / Android向けのホーム画面アプリ（PWA）に対応。[公開版を開く](https://relay-chi-ecru.vercel.app/)。SafariまたはChromeから追加します。`prototype/index.html` をファイルとして開く場合はインストール・オフライン機能を利用できません。[導入と検証手順](docs/MOBILE_APP.md)
