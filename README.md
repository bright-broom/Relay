...
flowchart TB
    ...
    nav --> customers["顧客台帳：所属内の顧客を登録・再取得"]
...
|---|---|---|
| 顧客台帳 | 所属選択、登録、一覧・詳細、再送制御、監査 | CRM 専用 DB と本人・所属の事前登録が必要。編集・アーカイブは未実装 |
...
「顧客台帳」で、所属先ごとの顧客登録・一覧・詳細の再取得を実装しました。CRM 専用の制限された DB 接続、本人・所属の事前登録が必要です。公開デモの架空案件は自動移行しません。本番適用・実顧客での受入は未実施です。[実装範囲・利用開始手順](docs/CRM_IMPLEMENTATION.md)

[CRM データベース設計](docs/CRM_DATABASE.md)の 14 テーブルのうち、本人・所属・顧客・監査・再送に必要な 6 テーブルを実装しています。顧客の編集、連絡先・案件・活動・タスク・料金提案の永続化は後続です。[設計用 SQL](docs/database/crm-draft.sql)は引き続き本番マイグレーションと分離します。

## 読み込み速度

日常の案件操作は先に読み込み、料金・日程・顧客台帳・マイページ・管理は必要時に読み込みます。メニューへのフォーカスでもコードを先読みします。初期 JavaScript は約 1.30 MB から約 0.50 MB に削減。実回線での表示時間は未測定です。[読み込み方針・計測方法・検証範囲](docs/PERFORMANCE.md)
...
| 顧客台帳・監査・再送記録 | サーバーの PostgreSQL / relay_crm | Google 本人確認・所属・RLS。CRM 専用の制限された接続 |
...
flowchart LR
    ...
    check --> generated["ソース差分と生成の再現性を確認"]
...
CRM の SQL・API・画面テストは `npm run check` に含みます。`npm run test:crm:postgres` では、Docker 上の一時 PostgreSQL を使って同時再送と所属停止の競合を検証します。CI でも両方を必須実行します。

説明文書だけの変更では重い検証 CI を起動しません。実行可能な `docs/database/` の SQL・検証コードは検証対象です。文書はコードとの整合、リンク、図の構文を確認します。[確認状況](docs/VALIDATION.md)
