...
実装済みの接続基盤。実サービスでの利用開始には、Google OAuth、PostgreSQL、LINE Messaging APIの設定が必要。設定が揃うまではログインを拒否する。一般画面と料金シミュレーターの公開デモは、未ログインでも利用できる。案件画面は引き続き架空データのプロトタイプであり、案件データのサーバー同期・期限の自動通知は未実装。
...
認証導入に伴い、HTML・アプリ・APIのオフライン配信を停止。新しいWorkerは以前のRelayキャッシュを削除し、開いている画面を一度再読み込みする。以後すべてネットワーク経由。認証済み画面は起動・復帰・表示中60秒ごとにセッション確認し、失効したら公開入口へ戻す。公開デモはセッションAPIを呼ばずに起動する。ホーム画面への追加は継続可能。

既に配布された公開デモやGitHubの架空サンプルは遡って秘密にはできない。古いインストールは一度オンラインで更新する。`file://` は架空データのローカルデザインプレビューとして残し、認証・LINEは使えない。実データや秘密情報を追加しない。

端末内の案件保存はサンプル用途。公開デモはsessionStorage、認証済み画面はlocalStorageを使い、公開時に以前の利用者の下書きを復元しない。認証済みの保存はアカウント切替・ログアウトで破棄する。実案件同期、組織単位の権限管理、担当者の期限通知、通知設定の細分化は次の実装範囲。
