...
> 本書は初期構想です。現在の実装は[README](../README.md)、顧客・案件の永続化に関する最新設計は[CRMデータベース設計](CRM_DATABASE.md)を優先してください。下記の初期スキーマ・スタック記述をそのまま本番仕様にしないでください。
...
2026-09-13追記：商品導入前後の一定月額・総支払額比較を追加した。[料金シミュレーション仕様](PRICING.md)を優先する。融資条件の算出、補助金の適格判定、営業評価、日報集計はこの初期版には含めない。ローンや補助金の案件でも「書類回収や確認の進行」は管理する。
