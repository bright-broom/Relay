...
朝、担当者は「今日の対応」を見る。案件を開くと、状況・次の対応・期限・元の発言が一画面で分かる。電話で進んだ仕事は短いメモで記録する。管理者は担当不明・期日接近・情報の食い違いだけを確認する。
...
最新の見た目・寸法・状態・共通部品の基準は [デザインガイド](DESIGN_GUIDELINES.md) を参照する。初期HTMLの配色・文字・余白を本番の正本としない。

PCでは左ナビゲーションと中央一覧を基本とし、案件詳細は情報量に応じてサイドパネルまたは専用ページを使う。初期HTMLの詳細ダイアログは画面フローの検討用。スマートフォンでは一列カードと全画面詳細。色に加えて「本日」「期日未設定」「完了確認が必要」など文字で状態を表す。
...
**電話 → 記録**：案件を開く → 「顧客に確認。9/18 15時で了承」とメモ → 予定確認済みの更新案を確認 → 承認。工事会社へ確定連絡が必要なら別タスクとして残す。
