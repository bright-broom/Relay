...
import {errorFields} from '../../tests/support.ts';
...
const insert = async (table:string, values:Record<string,unknown>) => {
 ...
 return pg.query<Record<string, unknown>>(`INSERT INTO relay_crm.${table} (${columns.join(',')}) VALUES (${columns.map((_, index) => '$' + (index + 1)).join(',')})`, Object.values(values));
};
const reject = async (work:()=>Promise<unknown>, code:string) => assert.rejects(work, error => errorFields(error).code === code);
try {
 ...
 const {rows: privateTables} = await pg.query<Record<string, unknown>>("SELECT count(*)::int AS n FROM pg_tables WHERE schemaname='relay_private'");
 ...
 await reject(() => pg.query<Record<string, unknown>>('UPDATE relay_crm.activities SET summary=$1 WHERE id=$2',['Overwrite',id(71)]), '55000');
 await reject(() => pg.query<Record<string, unknown>>('DELETE FROM relay_crm.activities WHERE id=$1',[id(71)]), '55000');
 ...
 const update = () => pg.query<Record<string, unknown>>('UPDATE relay_crm.customers SET region=$1,version=version+1,updated_at=now() WHERE workspace_id=$2 AND id=$3 AND version=1 RETURNING version',['Synthetic region',id(1),id(21)]);
 ...
 await reject(() => pg.query<Record<string, unknown>>('DELETE FROM relay_crm.proposal_snapshots WHERE id=$1',[id(91)]), '55000');
 ...
 await reject(() => pg.query<Record<string, unknown>>('UPDATE relay_crm.audit_events SET action=$1 WHERE id=$2',['forged',id(101)]), '55000');
 ...
 await reject(() => pg.query<Record<string, unknown>>('DELETE FROM relay_crm.customers WHERE id=$1',[id(21)]), '23503');
 await pg.query<Record<string, unknown>>("UPDATE relay_crm.memberships SET status='suspended' WHERE principal_id=$1",[id(11)]);
 assert.equal((await pg.query<Record<string, unknown>>('SELECT count(*)::int AS n FROM relay_crm.activities WHERE actor_id=$1',[id(11)])).rows[0].n,2);
 const {rows: tables} = await pg.query<Record<string, unknown>>("SELECT relname, relrowsecurity, relforcerowsecurity FROM pg_class JOIN pg_namespace ON pg_namespace.oid=relnamespace WHERE nspname='relay_crm' AND relkind='r'");
 ...
 assert.equal((await pg.query<Record<string, unknown>>('SELECT count(*)::int AS n FROM relay_crm.customers')).rows[0].n,0);
