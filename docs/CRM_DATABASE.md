...
以下は全体設計案。顧客台帳の登録・参照・編集・アーカイブ・復元は段階実装済みで、現在の契約は [CRM_IMPLEMENTATION.md](CRM_IMPLEMENTATION.md) を正本とする。実装ではアーカイブ・復元も顧客 PATCH の action として扱う。連絡先は新規登録と顧客別参照を段階実装済み。連絡先の編集・共有関係の変更、案件・活動・タスク・提案は未実装。公開デモ用の既存ルートに実顧客データを混ぜない。
