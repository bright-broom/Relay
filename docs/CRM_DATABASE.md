...
2026-09-14 / 全体設計案 v1 / **本番未適用**

2026-09-17 追記：本人・所属・顧客登録・一覧・再取得・監査・再送を第 1 段階として実装。以下は全体の設計契約であり、全機能の実装完了を意味しない。現在の実装・権限・設定・検証の正本は[顧客台帳の実装](CRM_IMPLEMENTATION.md)。
...
以下は全体設計案。顧客台帳の登録・参照・編集・アーカイブ・復元は段階実装済みで、現在の契約は [CRM_IMPLEMENTATION.md](CRM_IMPLEMENTATION.md) を正本とする。実装ではアーカイブ・復元も顧客 PATCH の action として扱う。連絡先は新規登録・顧客別参照・氏名と連絡手段の編集を段階実装済み。主連絡先・共有関係の変更、案件・活動・タスク・提案は未実装。公開デモ用の既存ルートに実顧客データを混ぜない。
