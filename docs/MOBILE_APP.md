...
公開URL：https://relay-chi-ecru.vercel.app/
...
- オフライン：認証導入に伴い停止。起動・復帰にはオンライン接続が必要。一般画面・料金シミュレーターの公開デモはログイン不要で、管理情報・外部連携は認証必須。古いRelayキャッシュは新しいWorkerで削除する。
...
2. 未ログインではタブ内の再読み込みでデモ記録が復元し、認証済みの下書きを読み込まないことを確認する。認証済みでは端末内保存の復元と、ログアウト・アカウント切替時の削除を確認する。
3. 機内モードでは認証を迂回してアプリが起動しないことを確認する。
4. ノッチ・ホームインジケーター・横向き・ソフトウェアキーボード・200%拡大時の操作を確認する。
5. 旧PWAから更新し、旧キャッシュの削除・再読み込みと、管理・外部連携へのログイン要求を確認する。
