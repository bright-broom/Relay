...
export async function handle(request: Request, db?: Database, authProvider?: Configuration, crmDb?: Database): Promise<Response> {
  ...
  try {
    ...
    if (route === 'crm-contact' || route === 'crm-contacts' || route === 'crm-customers' || route === 'crm-customer' || route === 'crm-search') {
      ...
      if (route === 'crm-contact') return Response.json(await editContact(identity, url.searchParams.get('id'), input, crmDb));
...
/** @public Invoked by Vercel function entry; source is loaded through esbuild. */
