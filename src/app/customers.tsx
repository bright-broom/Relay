...
export function Customers({ui}: {ui: UiContext}) {
  ...
  const listPath = (id = workspaceId, showArchived = archived) => `customers?workspaceId=${encodeURIComponent(id)}&archived=${showArchived}`;
  const refresh = () => void run(async request => {
    ...
    setWorkspaceId(next);
    if (next) setPage(await request<CustomerPage>(listPath(next)));
  });
  ...
  const changeWorkspace = (next: string) => {
    ...
    void run(async request=>setPage(await request<CustomerPage>(listPath(next))));
  };
  const save = () => {
    ...
    void run(async request => {
      ...
      setPending(null); setDisplayName(''); setSelected(result.customer); setSaved(true); setArchived(false);
      // A subsequent list failure must not undo the confirmed save notification.
      setPage(null);
      setPage(await request<CustomerPage>(listPath(attempt.workspaceId,false)));
    });
  };
        ...
        {member && <>
          ...
          {selected && <CustomerEditor key={`${selected.id}:${selected.version}`} ui={ui} customer={selected}
            ...
            onLock={setEditorLocked} onStart={()=>{setSaved(false);setChanged(null);}} onReviewed={customer=>{
              ...
              void run(async request=>setPage(await request<CustomerPage>(listPath())));
              ...
              setPage(null);
              setPage(await request<CustomerPage>(listPath()));
            }} />}
          <SelectField ui={ui} label="crmListState" value={archived ? 'archived' : 'current'} disabled={busy || !!pending || editorLocked}
            ...
            onChange={event=>{
              ...
              void run(async request=>setPage(await request<CustomerPage>(listPath(workspaceId,next))));
            }} />
          <section className="stack" aria-label={t('crmCustomers')}>
            {!page && !busy && workspaceId && <Action ui={ui} label="crmReloadList" variant="outline" disabled={!!pending || editorLocked} onClick={()=>void run(async request=>setPage(await request<CustomerPage>(listPath())))} />}
            {page?.customers.length===0 && <Notice>{t(archived ? 'crmEmptyArchived' : 'crmEmpty')}</Notice>}
            ...
            {page?.nextCursor && <Action ui={ui} label="crmNextPage" variant="outline" disabled={busy || !!pending || editorLocked}
              onClick={()=>void run(async request=>{
                setSelected(null);
                setPage(await request<CustomerPage>(`${listPath()}&cursor=${encodeURIComponent(page.nextCursor!)}`));
              })} />}
          </section>
        </>}
      </>}
