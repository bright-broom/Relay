...
type LinkItem={kind:'page';id:'today'|'cases'|'reviews'|'imports'|'admin';label:MessageKey;icon:string};
...
export function visibleNavigation(installed:boolean,admin=false){return navigationItems.filter(item=>(!installed||item.id!=='install-info')&&(item.id!=='admin'||admin));}
